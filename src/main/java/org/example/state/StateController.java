...
public class StateController {
    ...
    public void listByMember() {
        ...
        if (listMemberSelect.equals("특정")) {
            ...
            System.out.println("["+ memberName + "]님의 현재 근태는 ["+ member.getState()+"] 입니다.");
            stateService.listByMember(memberName);
        } else if (listMemberSelect.equals("전체")) {
            List<Member> memberList = memberRepository.findByState();
            System.out.println(" 【 사원번호 / 이름 / 부서 / 직급 / 근태 / 변경시간 】 ");
            System.out.println("  ---------------------------------------------");
            for (Member member : memberList) {
                System.out.println(" 【 "+member.getId()+" / "+member.getName()+" / "+member.getDeptId()+" / "+member.getPosition()+" / "+member.getState()+" / "+member.getModifiedDate()+" 】");
            }
        }
    }
